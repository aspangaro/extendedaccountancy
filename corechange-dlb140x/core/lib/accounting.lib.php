 ...
 *
 *	@param	integer		$isSellerInEEC  true = country inside EEC, false = country outside EEC
 *	@param	integer		$isBuyerInEEC   true = country inside EEC, false = country outside EEC
 *	@param 	integer	    $productid      Rowid of product - Can be empty, Default account are suggested
 *  @param  integer     $thirdpartyid   Rowid of thirdparty - Can be empty, needed for level 3
 *  @param  array       $otherinfo      Array with list of other info
 *
 *  @return	array		Accounting accounts suggested
 */
function getAccountingCodeToBind($isSellerInEEC = 1, $isBuyerInEEC = 1, $productId = '', $thirdpartyId = '', $otherInfo = array())
{
    ...
    $parameters = array();
    $result = $hookmanager->executeHooks('accoutancyBindingCalculation', $parameters); // Note that $action and $object may have been modified by some hooks
    ...
    if (is_numeric($result)) {
        if (empty($result)) {
            ...
            // Propose another calculation method to bind
        }
    }
